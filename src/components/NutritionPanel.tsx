...
export default function NutritionPanel({
  ...
  const panelSpring = useSpring({
    ...
    transform: hasData ? 'translateY(0px)' : 'translateY(10px)',
  });

  const nutritionItems = [
    { label: 'カロリー', value: nutritionFacts.calories.toFixed(1), unit: 'kcal', color: 'ferment-primary' },
    { label: 'タンパク質', value: nutritionFacts.protein.toFixed(1), unit: 'g', color: 'protein-primary' },
    { label: '炭水化物', value: nutritionFacts.carbohydrate.toFixed(1), unit: 'g', color: 'koji-primary' },
    { label: '脂質', value: nutritionFacts.fat.toFixed(1), unit: 'g', color: 'ferment-secondary' },
    { label: 'ナトリウム', value: nutritionFacts.sodium.toFixed(0), unit: 'mg', color: 'ferment-accent' },
          ...
          {hasData ? (
            ...
            >
              ...
              >
                ...
                <p className="text-2xl font-bold text-ferment-primary">
                  {fermentationPeriod}日
                </p>
              </motion.div>
...
function NutritionItem({ label, value, unit, color, index }: NutritionItemProps) {
  ...
  return (
    <animated.div style={itemSpring}>
      ...
      >
        <div className="text-xs text-ferment-secondary/80 mb-1 font-medium">
          {label}
        </div>
        <div className="flex items-baseline gap-1">
          ...
          <span className="text-sm text-ferment-secondary/70">
            {unit}
          </span>
        </div>
      </motion.div>
